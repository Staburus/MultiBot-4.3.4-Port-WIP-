...
MultiBot:SetScript("OnUpdate", function(pSelf, pElapsed)
	...
	if(MultiBot.auto.stats and MultiBot.timer.stats.elapsed >= MultiBot.timer.stats.interval) then
		for i = 1, MultiBot.GetPartySize() do SendChatMessage("stats", "WHISPER", nil, UnitName("party" .. i)) end
...
MultiBot:SetScript("OnEvent", function(pSelf, pEvent, pArg1, pArg2)
	local event = pEvent
	local arg1 = pArg1
	local arg2 = pArg2
	...
	if(event == "PLAYER_LOGOUT") then
		...
		MultiBotSave["CompactUI"] = MultiBot.IF(MultiBot.compact, "true", "false")
	...
	if(event == "ADDON_LOADED" and arg1 == "MultiBot") then
		...
		if(MultiBotSave["CompactUI"] ~= nil) then
			local tCompact = MultiBotSave["CompactUI"] ~= "false"
			local tButton = MultiBot.frames["MultiBar"].frames["Main"].buttons["Advanced"]
			
			MultiBot.SetCompactUI(tCompact)
			if(tButton ~= nil) then
				if(tCompact) then tButton.setDisable() else tButton.setEnable() end
			end
		end
	...
	if(event == "CHAT_MSG_SYSTEM") then
		if(MultiBot.isInside(arg1, "Accountlevel", "account level", "Account Level", "niveau de compte", "等级")) then
		...
		if(string.sub(arg1, 1, 12) == "Bot roster: ") then
			...
			if(GetNumRaidMembers() > 4) then
				for i = 1, GetNumRaidMembers() do
					...
					SendChatMessage(".playerbots bot add " .. tName, "SAY")
				end
			...
			if(MultiBot.GetPartySize() > 0) then
				for i = 1, MultiBot.GetPartySize() do
		...
		if(MultiBot.isInside(arg1, "player already logged in")) then
			...
			if(MultiBot.GetPartySize() == 4) then ConvertToRaid() end
