...
local tButton = tLeft.addButton("Quick", -34, 0, "inv_misc_enggizmos_27", MultiBot.tips.quick.master)
tButton.doLeft = function(pButton)
	MultiBot.ShowHideSwitch(pButton.parent.frames["Quick"])
end

local tQuick = tLeft.addFrame("Quick", -36, 34)
tQuick:Hide()

tQuick.addButton("Bots", 0, 0, "inv_scroll_04", MultiBot.tips.quick.bots)
.doLeft = function(pButton)
	local tButton = MultiBot.frames["MultiBar"].buttons["Units"]
	tButton.doLeft(tButton)
end

tQuick.addButton("Attack", 0, 30, "Interface\\AddOns\\MultiBot\\Icons\\attack.blp", MultiBot.tips.quick.attack)
.doLeft = function(pButton)
	if(MultiBot.isTarget()) then MultiBot.ActionToGroup("do attack my target") end
end

tQuick.addButton("Tank", 0, 60, "ability_warrior_shieldbash", MultiBot.tips.quick.tank)
.doLeft = function(pButton)
	if(MultiBot.isTarget()) then MultiBot.ActionToGroup("@tank do attack my target") end
end

tQuick.addButton("Follow", 0, 90, "Interface\\AddOns\\MultiBot\\Icons\\command_follow.blp", MultiBot.tips.quick.follow)
.doLeft = function(pButton)
	MultiBot.ActionToGroup("follow")
end

tQuick.addButton("Stay", 0, 120, "Interface\\AddOns\\MultiBot\\Icons\\command_stay.blp", MultiBot.tips.quick.stay)
.doLeft = function(pButton)
	MultiBot.ActionToGroup("stay")
end

tQuick.addButton("Passive", 0, 150, "Interface\\AddOns\\MultiBot\\Icons\\mode_passive.blp", MultiBot.tips.quick.passive).setDisable()
.doLeft = function(pButton)
	if(MultiBot.OnOffSwitch(pButton)) then
		MultiBot.ActionToGroup("co +passive,?")
	else
...
tQuick.addButton("Flee", 0, 180, "Interface\\AddOns\\MultiBot\\Icons\\flee.blp", MultiBot.tips.quick.flee)
.doLeft = function(pButton)
	MultiBot.ActionToGroup("flee")
end

tQuick.addButton("Summon", 0, 210, "ability_hunter_beastcall", MultiBot.tips.quick.summon)
.doLeft = function(pButton)
...
MultiBot.SetCompactUI = function(pCompact)
	local tLeft = MultiBot.frames["MultiBar"].frames["Left"]
	local tQuick = tLeft.buttons["Quick"]
	local tAdvanced = { "Tanker", "Flee", "Format" }
	
	MultiBot.compact = pCompact
	
	for i = 1, table.getn(tAdvanced) do
		local tButton = tLeft.buttons[tAdvanced[i]]
		local tFrame = tLeft.frames[tAdvanced[i]]
		if(tButton ~= nil) then
			if(pCompact) then tButton:Hide() else tButton:Show() end
		end
		if(tFrame ~= nil) then tFrame:Hide() end
	end
	
	if(tQuick ~= nil) then
		if(pCompact) then tQuick:Show() else tQuick:Hide() end
	end
	if(tLeft.frames["Quick"] ~= nil) then tLeft.frames["Quick"]:Hide() end
	
	return pCompact
end

MultiBot.SetCompactUI(true)
...
.doLeft = function(pButton)
	SendChatMessage(".playerbots bot addclass dk", "SAY")
end
...
tButton.doRight = function(pButton)
	if(GetNumRaidMembers() > 0) then
		for i = 1, GetNumRaidMembers() do
			...
			elseif(tName ~= UnitName("player")) then SendChatMessage(".playerbots bot init=auto " .. tName, "SAY")
			end
		end
...
tButton.doLeft = function(pButton)
	...
	SendChatMessage(".playerbots bot init=auto " .. tName, "SAY")
end
...
tButton.doRight = function(pButton)
	if(GetNumRaidMembers() > 0 or GetNumPartyMembers() > 0) then
		return SendChatMessage(".playerbots bot remove *", "SAY")
	else
...
if(MultiBot.tips.main.advanced == nil) then
	MultiBot.tips.main.advanced = "Advanced-Bar\n|cffffffffShows or hides the less-used combat controls.|r\n\n|cffff0000Left-Click to switch compact mode|r"
end

tMain.addButton("Advanced", 0, 340, "inv_misc_enggizmos_27", MultiBot.tips.main.advanced).setDisable()
.doLeft = function(pButton)
	if(MultiBot.OnOffSwitch(pButton)) then
		MultiBot.SetCompactUI(false)
	else
		MultiBot.SetCompactUI(true)
	end
end
